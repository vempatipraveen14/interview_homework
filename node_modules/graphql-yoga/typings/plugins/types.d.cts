import type { Instrumentation as EnvelopInstrumentation, Plugin as EnvelopPlugin, OnExecuteHook, OnSubscribeHook, PromiseOrValue, RegisterContextErrorHandler, SetSchemaFn } from '@envelop/core';
import type { ExecutionResult } from '@graphql-tools/utils';
import type { MaybePromise } from '@whatwg-node/promise-helpers';
import type { ServerAdapterInitialContext, Instrumentation as ServerAdapterInstrumentation, ServerAdapterPlugin } from '@whatwg-node/server';
import type { YogaServer } from '../server.cjs';
import type { FetchAPI, GraphQLHTTPExtensions, GraphQLParams, MaybeArray, YogaInitialContext } from '../types.cjs';
...
export type Instrumentation<TContext extends Record<string, any>> = EnvelopInstrumentation<TContext> & ServerAdapterInstrumentation & {
    operation?: (payload: {
        context: TContext;
        request: Request;
    }, wrapped: () => PromiseOrValue<void>) => PromiseOrValue<void>;
    requestParse?: (payload: {
        request: Request;
    }, wrapped: () => MaybePromise<void>) => MaybePromise<void>;
    resultProcess?: (payload: {
        request: Request;
    }, wrapped: () => MaybePromise<void>) => MaybePromise<void>;
};
...
export type RequestParser = (request: Request) => PromiseOrValue<GraphQLParams | GraphQLParams[] | Response>;
...
export type OnParamsHook<TServerContext> = (payload: OnParamsEventPayload<TServerContext>) => PromiseOrValue<void>;
...
export type ParamsHandler<TServerContext> = (payload: ParamsHandlerPayload<TServerContext>) => PromiseOrValue<ExecutionResult | AsyncIterable<ExecutionResult>>;
